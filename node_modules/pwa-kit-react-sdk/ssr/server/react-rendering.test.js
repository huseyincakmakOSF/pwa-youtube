...
var _buildRemoteServer = require("pwa-kit-runtime/ssr/server/build-remote-server");
var _supertest = _interopRequireDefault(require("supertest"));
var _nodeHtmlParser = require("node-html-parser");
var _path = _interopRequireDefault(require("path"));
var _ssrServer = require("pwa-kit-runtime/utils/ssr-server");
...
const mobile = 'Mozilla/5.0 (iPhone; U; CPU like Mac OS X; en) AppleWebKit/420+ (KHTML, like Gecko) Version/3.0 Mobile/1A543 Safari/419.3';
const tablet = 'Mozilla/5.0 (iPad; CPU OS 6_1 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10B141 Safari/8536.25';
...
describe('The Node SSR Environment', () => {
  ...
  const cases = [{
    ...
    assertions: res => {
      ...
      expect(doc.querySelector('body').getAttribute('class')).toEqual('helmet-body-attribute');
      ...
      expect(doc.querySelector('style').innerHTML.includes('background-color: blue;')).toBe(true);
      expect(doc.querySelector('noscript').innerHTML.includes('<link rel="stylesheet" type="text/css" href="foo.css" />')).toBe(true);
      ...
      expect(head.querySelector('script[src="http://include.com/pathtojs.js"]')).not.toBe(null);
      expect(head.querySelector('script[type="application/ld+json"]').innerHTML.includes(`"@context": "http://schema.org"`)).toBe(true);
    }
  }, {
  ...
  isRemoteValues.forEach(isRemoteValue => {
    ...
    cases.forEach(({
      ...
      test(`renders PWA pages properly when ${isRemoteValue ? 'remote' : 'local'} (${description})`, () => {
        ...
        return (0, _supertest.default)(app).get(url).set(headers || {}).query(query || {}).then(res => assertions(res));
      });
    });
  });
});