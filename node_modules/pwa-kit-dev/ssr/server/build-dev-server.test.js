...
var _constants = require("pwa-kit-runtime/ssr/server/constants");
var _ssrProxying = require("pwa-kit-runtime/utils/ssr-proxying");
...
describe('DevServer error handlers', () => {
  const testServerErrorHandler = (error, times) => {
    ...
    const log = jest.fn();
    const handler = (0, _buildDevServer.makeErrorHandler)(proc, devserver, log);
    const e = {
      code: error
    };
    handler(e);
...
describe('DevServer request processor support', () => {
  ...
  test('SSRServer handles no request processor', () => {
    ...
    app.get('/*', route);
    return (0, _supertest.default)(app).get('/').expect(200).then(response => {
      expect(response.headers[_ssrProxying.X_MOBIFY_REQUEST_CLASS]).toBe(undefined);
      expect(route).toHaveBeenCalled();
...
describe('DevServer persistent caching support', () => {
  ...
  const cachedRoute = route => (req, res) => {
    ...
    return Promise.resolve().then(() => getResponseFromCache(cacheArgs)).then(entry => {
      if (entry.found) {
        sendCachedResponse(entry);
      } else {
        if (shouldCache) {
          cacheResponseWhenDone(_objectSpread({
            shouldCacheResponse
          }, cacheArgs));
        }
    ...
    context: {
      stats: {
        stats: [{
          compilation: {
            name: 'server',
            assetsInfo: new Map([['static/favicon.ico', {
              sourceFilename: _path.default.resolve(testFixtures, 'app/static/favicon.ico')
            }]])
          }
        }]
      }
