...
const userAgents = {
  phone: {
    iphone5orSE: 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1',
    iphone6or7or8: 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
    iphone6or7or8plus: 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
    ...
    galaxyS5: 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3598.0 Mobile Safari/537.36',
    ...
    pixel2XL: 'Mozilla/5.0 (Linux; Android 8.0.0; Pixel 2 XL Build/OPD1.170816.004) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3598.0 Mobile Safari/537.36'
  },
  tablet: {
    ...
    nexus10: 'Mozilla/5.0 (Linux; Android 6.0.1; Nexus 10 Build/MOB31T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3598.0 Safari/537.36',
    ...
    ipadAir: 'Mozilla/5.0 (iPad; CPU OS 11_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/11.0 Mobile/15E148 Safari/604.1',
  ...
  desktop: {
    chrome72: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3598.0 Safari/537.36',
    firefox63: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.13; rv:63.0) Gecko/20100101 Firefox/63.0',
...
describe('utils/ssr-server tests', () => {
  test('getFullRequestURL', () => {
    expect((0, _ssrServer.getFullRequestURL)('https://a.b/c')).toEqual('https://a.b/c');
    expect(() => (0, _ssrServer.getFullRequestURL)('/somepath')).toThrow();
    (0, _ssrShared.updatePackageMobify)(baseMobify);
    expect((0, _ssrServer.getFullRequestURL)(`${_constants.PROXY_PATH_PREFIX}/base/somepath`)).toEqual('https://www.merlinspotions.com/somepath');
    expect((0, _ssrServer.getFullRequestURL)(`${_constants.PROXY_PATH_PREFIX}/base2/somepath`)).toEqual('https://api.merlinspotions.com/somepath');
  });
  describe('updatePackageMobify tests', () => {
    ...
    const testCases = [{
      ...
      validate: () => {
        ...
        expect(_ssrShared.proxyConfigs.length).toBe(inputConfigs.length);
        for (let i = 0; i < inputConfigs.length; i++) {
          ...
          expect(_ssrShared.proxyConfigs[i].proxyPath).toEqual(`/mobify/proxy/${inputConfigs[i].path}`);
          expect(_ssrShared.proxyConfigs[i].cachingPath).toEqual(`/mobify/caching/${inputConfigs[i].path}`);
        }
      }
    }, {
      ...
      validate: () => {
        expect(_ssrShared.proxyConfigs.length).toBe(3);
        expect(_ssrShared.proxyConfigs.every(config => config.protocol === 'http')).toBe(true);
        expect(_ssrShared.proxyConfigs[0].host).toEqual('somewhere.else');
        expect(_ssrShared.proxyConfigs[1].host).toEqual('another.place');
        expect(_ssrShared.proxyConfigs[2].host).toEqual('far.away');
        expect(_ssrShared.proxyConfigs[0].path).toEqual('base');
        expect(_ssrShared.proxyConfigs[1].path).toEqual('base9');
        expect(_ssrShared.proxyConfigs[2].path).toEqual('base8');
        expect(_ssrShared.proxyConfigs[0].proxyPath).toEqual('/mobify/proxy/base');
        expect(_ssrShared.proxyConfigs[1].proxyPath).toEqual('/mobify/proxy/base9');
        expect(_ssrShared.proxyConfigs[2].proxyPath).toEqual('/mobify/proxy/base8');
        expect(_ssrShared.proxyConfigs[0].cachingPath).toEqual('/mobify/caching/base');
        expect(_ssrShared.proxyConfigs[1].cachingPath).toEqual('/mobify/caching/base9');
        expect(_ssrShared.proxyConfigs[2].cachingPath).toEqual('/mobify/caching/base8');
      }
    }, {
      ...
      validate: () => {
        expect(_ssrShared.proxyConfigs.length).toBe(0);
      }
    }, {
  ...
  describe('detectDeviceType', () => {
    ...
    ...Object.keys(userAgents.phone).map(key => ({
      ...
      headers: {
        'user-agent': userAgents.phone[key]
      },
      expected: _ssrServer.PHONE
    })),
    // Test all tablet user agents
    ...Object.keys(userAgents.tablet).map(key => ({
      name: `test for user agent key ${key}`,
      headers: {
        'user-agent': userAgents.tablet[key]
      },
      expected: _ssrServer.TABLET
    })),
    // Test all desktop user agents
    ...Object.keys(userAgents.desktop).map(key => ({
      name: `test for user agent key ${key}`,
      headers: {
        'user-agent': userAgents.desktop[key]
      },
      expected: _ssrServer.DESKTOP
    })), {
      ...
      headers: {
        'CloudFront-Is-Mobile-Viewer': 'true',
        // Tablets are also mobile devices
    ...
    tests.forEach(testConfig => {
      test(`detectDeviceTypes (${testConfig.name})`, () => {
        ...
        expect((0, _ssrServer.detectDeviceType)(req)).toEqual(testConfig.expected);
      });
    });
    test('should allow users to force a device type, using the "mobify_devicetype" query param', () => {
      const headers = {
        'user-agent': userAgents.phone.iphoneX
      };
      const req = {
        get: key => headers[key],
        query: {
          mobify_devicetype: _ssrServer.TABLET
        }
      };
      expect((0, _ssrServer.detectDeviceType)(req)).toEqual(_ssrServer.TABLET);
    });
  });
});
...
test('escapeJSText', () => {
  expect((0, _ssrServer.escapeJSText)()).toEqual(undefined);
  expect((0, _ssrServer.escapeJSText)('</script>')).toEqual('\\x3c\\x2fscript>');
});
...
test('processLambdaResponse with no parameter', () => {
  ...
  expect((0, _ssrServer.processLambdaResponse)(null)).toBe(null);
});
describe('processExpressResponse', () => {
  const testCases = [{
    ...
    validate: headers => {
      expect(headers[_constants.CONTENT_TYPE]).toEqual('text/plain');
    ...
    validate: headers => {
      ...
      expect(headers[_constants.CONTENT_ENCODING]).toEqual('gzip');
      expect(headers[_constants.X_ORIGINAL_CONTENT_TYPE]).toEqual('text/plain');
    }
  }, {
  ...
  responseTypes.forEach(responseType => testCases.forEach(testCase => test(`${testCase.name} (${responseType.name})`, () => {
